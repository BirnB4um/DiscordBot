    ...
    if vc != None:
        if not PLAYING_SOUND:
            if sound in list(sound_files.keys()):
                vc.play(FFmpegPCMAudio(sound_files[sound]))
                PLAYING_SOUND = True
...
async def spruch(ctx, amount=1):
    ...
    for number in range(amount):
        await ctx.send(random.choice(sprueche_list))


@bot.command(name='witz', help=' - get random witz (.witz [amount])')
async def witz(ctx, amount=1):
    log(f">> {ctx.author.name}: {ctx.message.content}") 
    ...
    for number in range(amount):
        await ctx.send(random.choice(witze_list))


@bot.command(name='get_file', help=' - get file from path (.file [path])')
async def get_file(ctx, path=""):
    log(f">> {ctx.author.name}: {ctx.message.content}") 

    if ctx.author.id == user_id["thimo"]:
        if not (path == ""):
            if os.path.isfile("C:/Server/" + path):
                await ctx.send(f"C:/Server/{path}", file=discord.File("C:/Server/" + path))
            else:
                await ctx.send("file doesnt exist")
    else:
        await ctx.send("nenene so nich mein Freund")


@bot.command(name='shutdown', help=' - trigger bot shutdown (.shutdown confirm)')
async def shutdown(ctx, conf=""):
    global SHUTDOWN
    log(f">> {ctx.author.name}: {ctx.message.content}")

    if ctx.author.id == user_id["thimo"]:
        if conf.lower() == "confirm":
            await ctx.send("you need to confirm shutdown with '.shutdown confirm'")
            return
...
@bot.command(name='wassertemp', help=' - get wassertemparatur (.wassertemp)')
async def wassertemp(ctx):
    log(f">> {ctx.author.name}: {ctx.message.content}")

    page_response = requests.get('http://www.wassertemperatur.org/deutschland/bayern/sulzberger-see/', headers={'User-Agent': 'Chrome'})
    soup = BeautifulSoup(page_response.text, features="html.parser")
    wassertemp = soup.find(class_ = 'entry-content').h2.text
    await ctx.send(wassertemp)
...
async def words_with(ctx, letters=""):
    log(f">> {ctx.author.name}: {ctx.message.content}")
...
@bot.command(name='teamviewer', help=' - start teamviewer (.teamviewer)')
async def teamviewer(ctx):
    log(f">> {ctx.author.name}: {ctx.message.content}")

    if ctx.author.id == user_id["thimo"]:
        os.system("C:\Programme\TeamViewer\TeamViewer.exe")
    else:
        await ctx.send("nenene so nich mein Freund")
...
@bot.command(name='restart_server', help=' - restart server (.restart_server)')
