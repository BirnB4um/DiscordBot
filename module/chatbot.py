...
class DiscordChatbot:
    ...
    def __init__(self):
        ...
        self.tokenizer = Tokenizer.from_file("data/discord_tokenizer.json")
    ...
    def run_continuous(self, n_tokens_to_generate=50):
        ...
        with self.lock:
            ...
            self.is_running = True
        try:
            input_tokens = np.array(self.tokenizer.encode(self.TOKENS["end_of_message"]).ids).astype(np.int64).reshape(1, -1)
            output_list = []
...
class FourchanChatbot:
    ...
    def __init__(self):
        ...
        self.chatbot = ort.InferenceSession("data/4chan_chatbot_model.onnx")
        self.tokenizer = Tokenizer.from_file("data/4chan_tokenizer.json")
        self.sep_token_index = int(self.tokenizer.token_to_id(self.TOKENS["seperator"]))
    ...
    def run(self, prompt, n_tokens_to_generate=50):
        ...
        try:
            ...
            for i in range(n_tokens_to_generate):
                ...
                if output_list[-1] == self.sep_token_index:
                    break
                if i == 49:
                    output_list[-1] = self.sep_token_index
                    break
            

            #feed EOM token back into model
            ort_inputs = {
                "input_tokens": np.array(self.sep_token_index).astype(np.int64).reshape(1, 1),
                "hidden_state_in": self.hidden_state,
                "cell_state_in": self.cell_state
            }
            ort_outputs = self.chatbot.run(None, ort_inputs)
            ...
            generated_message = self.tokenizer.decode(output_list)
            generated_message = generated_message.replace(self.TOKENS["seperator"], "")
    ...
    def run_continuous(self, n_tokens_to_generate=50):
        ...
        with self.lock:
            ...
            self.is_running = True
        try:
            input_tokens = np.array(self.tokenizer.encode(self.TOKENS["seperator"]).ids).astype(np.int64).reshape(1, -1)
            output_list = []
